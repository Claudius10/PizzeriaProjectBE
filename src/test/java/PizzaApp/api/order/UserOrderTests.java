...
public class UserOrderTests {
	...
	public void setup() {
		...
		orderIdMinus11Mins = Long.valueOf(createTestSubject(11));
		orderIdMinus16Mins = Long.valueOf(createTestSubject(16));
		orderIdMinus21Mins = Long.valueOf(createTestSubject(21));
		orderToDelete = Long.valueOf(createTestSubject(0));
	}
	...
	public void givenUserOrder_whenCreate_thenReturnOrder() throws Exception {
		...
		Order dbOrder = ordersService.findById(Long.valueOf(mockMvc.perform(post("/api/user/orders")
	...
	public void givenNewUserData_whenUpdate_thenReturnOrderWithNewData() throws Exception {
		...
		// action: persist and retrieve order
		Order dbOrder = ordersService.findById(Long.valueOf(mockMvc.perform(put("/api/user/orders")
						.contentType(MediaType.APPLICATION_JSON)
		...
		// then expect/assert: returned data matches set data
		assertAll("Data returned matches expected values",
				() -> assertEquals(dbOrder.getId(), orderUpdate.getOrderId()),
				() -> assertEquals(dbOrder.getCustomerName(), orderUpdate.getUserOrderData().customerName()),
				() -> assertEquals(dbOrder.getContactTel(), orderUpdate.getUserOrderData().tel()),
				() -> assertEquals(dbOrder.getEmail(), orderUpdate.getUserOrderData().email()));
	...
	public void givenNewAddress_whenUpdate_thenReturnOrderWithUpdatedAddress() throws Exception {
		...
		assertAll("Data returned matches expected values",
				() -> assertEquals(dbOrder.getId(), orderUpdate.getOrderId()),
				() -> assertEquals(dbOrder.getAddress().getId(), orderUpdate.getUserOrderData().addressId())
		);
	...
	public void givenOrderDetails_whenUpdate_thenReturnOrderWithNewOrderDetails() throws Exception {
		...
		assertAll("Data returned matches expected values",
				() -> assertEquals(dbOrder.getId(), orderUpdate.getOrderId()),
				() -> assertTrue(dbOrder.getOrderDetails().entityEquals(orderUpdate.getOrderDetails()))
		);
	...
	public void givenCart_whenUpdate_thenReturnOrderWithNewCart() throws Exception {
		...
		assertAll("Data returned should match data set for update",
				() -> assertEquals(dbOrder.getId(), orderUpdate.getOrderId()),
				() -> assertTrue(dbOrder.getCart().entityEquals(orderUpdate.getCart()))
		);
	...
	public void givenOrderDeleteAfterTimeLimit_whenDelete_thenThrowException() throws Exception {
		...
		// action: update order
		mockMvc.perform(delete("/api/user/orders/{orderId}", orderIdMinus21Mins)
						.cookie(SecurityCookieUtils.makeCookie("fight", validAccessToken, 30, true, false))
						.cookie(SecurityCookieUtils.makeCookie("id", "1", 30, false, false))
						.with(csrf()))
				.andExpect(status().isBadRequest());
	...
	public void givenOrderDelete_whenDelete_thenDeleteOrder() throws Exception {
		...
		// action: update order
		mockMvc.perform(delete("/api/user/orders/{orderId}?userId=1", orderToDelete)
						.cookie(SecurityCookieUtils.makeCookie("fight", validAccessToken, 30, true, false))
						.cookie(SecurityCookieUtils.makeCookie("id", "1", 30, false, false))
						.with(csrf()))
	...
	public String createTestSubject(int minusMins) {
		UserOrderDTO order = new UserOrderDTO(
				new UserOrderDataDTO(
						1L,
						1L,
